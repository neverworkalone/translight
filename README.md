...
Translight translates English into Korean. It respects an applicable `lang="en"` declaration, while undeclared Latin text must contain multiple independent English clues or a distinctive short-label clue before it is sent to Chrome Translator. Common or repeated words shared with other Latin-script languages do not qualify by themselves. This conservative policy may leave ambiguous short English labels such as `Go`, `Text`, or `Page` unchanged, while labels such as `Breaking news`, `Access denied`, and `Search results` remain eligible. Undeclared text without sufficient evidence is left unchanged instead of being guessed as English; this avoids passing Spanish, French, German, and other Latin-script languages to the provider's English source-language model.
